...
function postprocess(target)
    ...
    for line in split(read(target, String), '\n')
        ...
        is_bitmask = startswith(line, r"@enum (xkb_state_component|xkb_state_match|xkb_x11_setup_xkb_extension_flags)")
        is_bitmask && (line = replace(line, r"@enum (xkb_state_component|xkb_state_match|xkb_x11_setup_xkb_extension_flags)" => s"@bitmask \1"))
        final *= line * '\n'
    end
    write(target, final)
end
